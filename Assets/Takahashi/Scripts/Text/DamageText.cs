...
public class DamageText : MonoBehaviour
{
    ...
    [Header("ダメージ量によるサイズ変化")]
    [Tooltip("このダメージ以上で少し大きく表示")]
    public int mediumDamageThreshold = 50;
    [Tooltip("このダメージ以上でさらに大きく表示")]
    public int highDamageThreshold = 100;
    [Tooltip("50〜99ダメージ時のサイズ倍率（フォント・アニメーション全体）")]
    public float mediumDamageSizeMultiplier = 1.2f;
    [Tooltip("100ダメージ以上のサイズ倍率（フォント・アニメーション全体）")]
    public float highDamageSizeMultiplier = 1.5f;

    // SetDamage()で決定し、フォントサイズ・アニメーションスケール両方に使う倍率
    ...
    public void SetDamage(int damage)
    {
        ...
        // ★追加：ダメージ量に応じてサイズ倍率を決定（フォント・アニメーション全体で共通利用）
        ...
        {
            damageSizeMultiplier = 1f; // 10〜49（またはそれ未満）は今まで通り
        }
    ...
    public void SetCritical()
    {
        ...
        Color c = new Color(1f, 0.5f, 0f); // オレンジ
        c.a = baseAlpha;
        text.color = c;

        text.fontStyle = FontStyles.Bold;

        // ★変更：ダメージ量サイズとクリティカル倍率を掛け合わせる
